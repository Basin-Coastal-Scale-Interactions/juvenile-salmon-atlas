...
read_csv(here::here("data-raw", "spatial", "grid_extended20241211.csv")) %>% glimpse()


# plotting grid
ggplot(grid_list) + geom_raster(aes(X, Y, fill = depth))
ggplot(grid_list) + geom_raster(aes(X, Y, fill = coast_distance_meter))
...
gridsp <- st_as_sf(grid_list, coords = c("X", "Y"), 
                   crs =6338, #32609, #26909, # UTM Zone 9M, sp::CRS("+proj=utm +zone=9 +units=m"),
                   remove = FALSE)
polysp  <- st_polygon(list(as.matrix(subpoly[, -3]))) # removing third column with labels

interiorsf <- fmesher::fm_segm(sdmTMB_mesh$mesh, boundary = FALSE) %>% 
...
gridsp_sub %>% #gridsp_sub
  ...
  geom_polygon(data = sfheaders::sf_to_df(interiorsf)[, 3:4]*1000, aes(x, y), fill = NA, color = "black") +
