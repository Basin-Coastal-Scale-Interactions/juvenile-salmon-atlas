...
pred_cmb <- readRDS(here("data", "pred_cmb_11groups.rds"))
...
pabs_slowmigs <- grid_slowmigs %>%
  ...
  scale_y_continuous(name = NULL, limits = range(grid_df$Yr)+c(-1000,1000), expand = c(0, 0)) +
  facet_grid(salmon_region ~ month_f)  +
  ggtitle("Distribution of juvenile chinook - \"Slow migration\" stock groupings") +
  NULL
...
pabs_fastmigs <- grid_fastmigs %>%
  ...
  scale_y_continuous(name = NULL, limits = range(grid_df$Yr)+c(-1000,1000), expand = c(0, 0)) +
  facet_grid(salmon_region ~ month_f)  +
  ggtitle("Distribution of juvenile chinook - \"Fast migration\" stock groupings") +
  NULL
